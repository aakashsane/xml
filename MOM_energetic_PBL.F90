...
! adding for neural network > start
...
type, public :: energetic_PBL_CS ; private
  ...
  !/ neural network model paramters
  logical :: NN, NN_copy  ! neural network - true or false
  character(len=200) :: NNfile   ! The name of netcdf file having neural network shape function
  character(len=200) :: NN_vel_file  ! name of netcdf file having neural network velocity
  real :: NN_hbl_lowlimit, NN_hbl_upplimit

  real, dimension(4) :: NN_force_mean
  real, dimension(4) :: NN_force_std
  real, dimension(16):: NN_kappa_mean
  real, dimension(16):: NN_kappa_std

  real, dimension(4, 32) :: NN_w1
  real, dimension(32) :: NN_b1
  real, dimension(32,32) :: NN_w2
  real, dimension(32) :: NN_b2
  real, dimension(32,16) :: NN_w3
  real, dimension(16) :: NN_b3

  real, dimension(3) :: NN_vel_force_mean
  real, dimension(3) :: NN_vel_force_std
  real, dimension(2) :: NN_vel_nubyh_mean
  real, dimension(2) :: NN_vel_nubyh_std

  real, dimension(3, 32) :: NN_vel_w1
  real, dimension(32) :: NN_vel_b1
  real, dimension(32,32) :: NN_vel_w2
  real, dimension(32) :: NN_vel_b2
  real, dimension(32,2) :: NN_vel_w3
  real, dimension(2) :: NN_vel_b3

  real :: LV
  !real, dimension(16) :: kappa
...
! The following are only used for diagnostics.
  ...
  ! For neural network diagnosis
  real :: LV_dummy ! dummy for LV, velocity scale
  ...
  do OBL_it=1,CS%Max_MLD_Its
    ...
    if (.not. OBL_converged) then
      ...
      elseif (MLD_guess <= 0.0) then
       ...
       call getshapefunction(CS,GV,h,absf,B_flux,u_star,MLD_guess,MixLen_shape)
          CS%shape_function=MixLen_shape
          !CS%kappa=0.0
          if (CS%NN == .true.) then
              CS%LV = 0.0 ! sets L times V to zero
              LV_dummy = 0.0 
              call get_neural_LV(CS,absf,B_flux,u_star,LV_dummy)
              CS%LV = LV_dummy 
          end if
         ...
         ! in unstable interior regions?
          ...
          if (TKE_here > 0.0) then
            ...
            if (.not.CS%Use_MLD_iteration) then
              Kd_guess0 = vstar * CS%vonKar * ((h_tt*hbs_here)*vstar) / &
                ((CS%Ekman_scale_coef * absf) * (h_tt*hbs_here) + vstar)
            else if (CS%NN == .true.) then  ! update Kd as per neural network here
                  Kd_guess0 = MixLen_shape(K) * CS%LV * MLD_guess  !  u_star * MLD_guess * 0.068
                  !print*,'TKE_here>0 line 1090, line1/3 , value of u_star is >',u_star
                  ! 0.068 * u * h is a very rough approximation of L* V* , new LV added
            else
            ...
            if (PE_chg_max <= 0.0) then
              ...
              if (TKE_here > 0.0) then
                ...
                !  instead of redoing the computation will change answers...
                  ...
                  Kd(K) = MixLen_shape(K) * CS%LV * MLD_guess  !u_star * MLD_guess * 0.068
                  !print*,'covective unstable, line2/3 , value of u_star is >',u_star
                  ! 0.068 * u * h is a very rough approximation of L* V* , new LV added
          ...
          elseif (tot_TKE == 0.0) then
            ...
            enddo ! Inner iteration loop on itt.
             ...
             !removed line3/3 as Brandon suggested it would not satisfy energy contraints            
           ! if (CS%NN == .true.) then  ! update Kd as per neural network here
           !       Kd(K) = MixLen_shape(K) * u_star * MLD_guess * 0.068
            !print*,'the above line 1304 , line3/3 ,  is updating Kd to give NN model'
           ! else 
            Kd(K) = Kddt_h_guess / dt_h ; Kddt_h(K) = Kd(K) * dt_h
           ! print*,'kd update from original epbl' 
           ! end if
...
subroutine get_neural_LV(CS,absf,B_flux,u_star,LV_dummy)
      ...
      real, intent(inout) :: LV_dummy   ! only velocity is the output. need to rename this later
      integer :: n ! integer for looping, local only for this routine    
      real, dimension(3)  :: x1 ! input vector
      real, dimension(32) :: x2 ! hidden layer 1
      real, dimension(32) :: x3 ! hidden layer 2
      real, dimension(2) :: x4  ! matmul(x3, CS%NN_vel_w3)

      !real :: u_rand ! random number for stochastic velocity part
      if (x1(1) .le. 0.000001) then
              x1(1) = 0.000001 - CS%NN_vel_force_mean(1)
      else 
              x1(1) = absf-CS%NN_vel_force_mean(1)
      endif
      x1(1) = x1(1)/CS%NN_vel_force_std(1)

      if (x1(2) .le. -7.0E-07) then
              x1(2) = -7.0E-07 - CS%NN_vel_force_mean(2)
      elseif (x1(2) .ge. 7.0E-07) then
              x1(2) =  7.0E-07 - CS%NN_vel_force_mean(2)
      else 
              x1(2) = (-1*B_flux) - CS%NN_vel_force_mean(2)
      endif
      x1(2) = x1(2)/CS%NN_vel_force_std(2)

      !x1(3) = u_star - CS%NN_vel_force_mean(3)
      
      if (x1(3) .ge. 0.139) then
              x1(3) = 0.139 - CS%NN_vel_force_mean(3)
      else 
              x1(3) = u_star - CS%NN_vel_force_mean(3)
      endif
      x1(3) = x1(3)/CS%NN_vel_force_std(3)

      !print*,' force input is ', x1 
      x2=matmul(x1,CS%NN_vel_w1)
      !x2=x2+CS%NN_b1
      do n=1,32
      x2(n)=x2(n)+CS%NN_vel_b1(n)
      end do
      call relu(x2,32)

      x3=matmul(x2,CS%NN_vel_w2)
  !x3=x3+CS%NN_b2
      do n=1,32
         x3(n)=x3(n)+CS%NN_vel_b2(n)
      end do
      call relu(x3,32)

      !LV=matmul(x3,CS%NN_vel_w3)
      x4=0.0
      LV_dummy=0.0
      x4=matmul(x3,CS%NN_vel_w3)

      do n=1,2
          x4(n)=x4(n)+CS%NN_vel_b3(n)
      !    LV_dummy = LV_dummy + x4(1)
      end do
      
      do n=1,2
          x4(n)=x4(n)*CS%NN_vel_nubyh_std(n) + CS%NN_vel_nubyh_mean(n)
      end do
      ...
      !x4=matmul(x3,CS%NN_vel_w3)
      !do n=1,2
      !    x4(n)= x4(n)*CS%NN_vel_nubyh_std(1) + CS%NN_vel_nubyh_mean(1)
      !end do
      !call rand_stdnormal(u_rand) random number does not work. The EPBL energy does not converge.
      ! also, the random number might have large -ve amplitude which makes viscosity negative
      ! so having a random component of velocity is a horrible idea
      !print*,' Gaussian random number is ', u_rand
      !LV_dummy=LV_dummy*CS%NN_vel_nubyh_std(1) + CS%NN_vel_nubyh_mean(1) 
      !LV_dummy = EXP(LV_dummy) !+ (u_rand)*EXP(x4(2))
      LV_dummy=max(LV_dummy,0.0001)

end subroutine get_neural_LV
...
subroutine getshapefunction(CS,GV,h,absf,B_flux,u_star,MLD_guess,MixLen_shape)
     ...
     ! integer, intent(in) :: szkgv
      ...
      if (CS%NN == .true.) then  ! update Kd as per neural network here
           hz=0.0
           mld_index=0
           !call get_mld_index_hz(GV, h, hz, MLD_guess, mld_index)
           !    kappa_neural(Kd, GV, h, hz, MLD_guess, k_points, mld_index)
           call kappa_neural(MixLen_shape, CS, GV, h, absf, B_flux, u_star, MLD_guess, 16)
           ! the call kappa_neural(Kd, GV, h, MLD_guess, k_points), right now, k_points=16
      else
      do K=2,nz+1
...
subroutine kappa_neural(shape_func, CS, GV, h, absf, B_flux, u_star, MLD_guess, k_points)
  type(verticalGrid_type), intent(in)    :: GV     !< The ocean's vertical grid structure.
  type(energetic_PBL_CS),  intent(in) :: CS     !< Energetic PBL control struct
  ...
  integer :: n ! integer for looping, local only
  ...
  ! mean shape function, will be replaced with actual later
  real, dimension(4)  :: x1 ! input vector
  real, dimension(32) :: x2 ! hidden layer 1
  real, dimension(32) :: x3 ! hidden layer 2
  real, dimension(k_points) :: kappa  ! shape function from NN model

  ! note that there 0 nad mld_index also exist in the shape function
  real, dimension(k_points+3) :: SF  ! shape function on sigma grid i.e. w grid
  
  ! initializing SF_z to zero, will be updated later in lin_interp1d
  !do n=1,mld_index 
  !  SF_z(n)=0.0
  !end do
  ...
  !print *,'B_flux value is >', B_flux
  ! using feedforward neural: 
  x1(1) = absf-CS%NN_force_mean(1)
  !print *,'absf value is', absf
  x1(1) = x1(1)/CS%NN_force_std(1)
  
  ! limits on B_flux set here
  if (x1(2) .le. -1.7505E-07) then   !-3.4E-08) then
          x1(2) = -1.7505E-07 - CS%NN_force_mean(2)
  elseif (x1(2) .ge. 1.7505E-07) then
          x1(2) = 1.7505E-07 - CS%NN_force_mean(2)
  else 
          x1(2) = (-1*B_flux) - CS%NN_force_mean(2)
  endif
  x1(2) = x1(2)/CS%NN_force_std(2)
  ! bflux input set.  
  
  ! setting limit on ustar
  if (x1(3) .ge. 0.040) then  !0.0312) then
          x1(3) = 0.040 - CS%NN_force_mean(3)
  else 
          x1(3) = u_star - CS%NN_force_mean(3)
  endif
  x1(3) = x1(3)/CS%NN_force_std(3)
  ! ustar input set 

  ! hbl input to neural network checked and modified here (start)
  if (MLD_guess .le. CS%NN_hbl_lowlimit) then
          x1(4) = CS%NN_hbl_lowlimit - CS%NN_force_mean(4)
  elseif (MLD_guess .ge. CS%NN_hbl_upplimit) then
          x1(4) = CS%NN_hbl_upplimit - CS%NN_force_mean(4)
  else
          x1(4) = MLD_guess - CS%NN_force_mean(4)
  end if
  x1(4) = x1(4)/CS%NN_force_std(4)
  ! hbl input to neural network checked and modified here (end)

  !print*,' force input is ', x1 
  x2=matmul(x1,CS%NN_w1)
  !x2=x2+CS%NN_b1
  do n=1,32
     x2(n)=x2(n)+CS%NN_b1(n)
  end do
  call relu(x2,32)

  x3=matmul(x2,CS%NN_w2)
  !x3=x3+CS%NN_b2
  do n=1,32
     x3(n)=x3(n)+CS%NN_b2(n)
  end do
  call relu(x3,32)

  kappa=matmul(x3,CS%NN_w3)
  !kappa=kappa+CS%NN_b3
  do n=1,16
     kappa(n)=kappa(n)+CS%NN_b3(n)
  end do

  do n=1,k_points
      kappa(n)=(kappa(n)*CS%NN_kappa_std(n))+CS%NN_kappa_mean(n)
      kappa(n)=EXP(kappa(n))
  end do

  !print *,' kappa is > ',kappa

  ! end of feedforward neural
  ...
  SF(k_points+2)=SF(k_points+1)*0.1
  ...
  ! creating the sigma coord on z grid from 0 to MLD_guess
  ...
  do n=2,k_points+1
      w(n)=w(n-1) + (MLD_guess - hz(0))/(k_points+1)
  end do  
  !w(k_points+2)=0.0 this is THE error, so commenting it out
  !do n=2,k_points+2   ! creating sigma to z grid.
  !    w(n)=w(n-1)+ (hz(mld_index)-hz(1))/(k_points+1)
  !end do

  ! now, SF = f(w), w is on z grid. woohoo!
  !call lin_interp1d(hz(1:mld_index),SF_z, w, SF, mld_index, k_points+2)
  !print *,'SF_z is > ',SF_z
  
  ! directly getting shape_function by interpolating
  ...
  !    shape_func(n)=SF_z(n)
  !end do
  !shape_func(mld_index)=SF_z(mld_index-1)*0.5 ! 10% of value at (mld_index-1)
  !do n=mld_index+1,SZK_(GV)+1
      !shape_func(n)=maxval(SF_z)*0.0001
  !    shape_func(n)=shape_func(mld_index)*0.5 ! shape_func below mld is 1% of value
     ! the above might smooth out Kd values near and below MLD, hopefully :|  
  !end do
  !print*,' mld_index is > ', mld_index
  !print*,' shape function value is', shape_func

end subroutine kappa_neural
...
subroutine get_mld_index_hz(GV, h, hz, MLD_guess, mld_index)
  ! gives us hz and mld_index
  ! hz is depth, obtained by integrated h
  type(verticalGrid_type), intent(in)    :: GV     !< The ocean's vertical grid structure.
  real, dimension(SZK_(GV)+1), intent(inout) :: hz ! depth variable, only used in this routine
  real, intent(in) :: MLD_guess !Mixing Layer depth guessed/found for iteration [Z ~> m].
  real, dimension(SZK_(GV)),intent(in) :: h   ! The layer thickness [H ~> m or kg m-2].
  integer, intent(inout) :: mld_index
  integer :: n
  hz(1)=0.0
  do n=2,SZK_(GV)+1
      hz(n)=hz(n-1) + h(n-1)*GV%H_to_Z
      if ((MLD_guess .ge. hz(n-1)) .and. (MLD_guess .lt. hz(n))) then
           !mld_index = n+1  ! mistake
           ! refining how mld_index is found: 
           ! if it is nearer to hz(n-1), the it is n-1
           ! if nearer to hz(n), then it is n
           if (MLD_guess .le. (hz(n-1)+hz(n))*0.5 ) then
                   mld_index = n-1
           else
                   mld_index = n
           end if
...
end subroutine get_mld_index_hz
...
!/Options related to Neural Network model
   call get_param(param_file, mdl, "NEURAL_NETWORK", CS%NN, &
                 "testing parameter file", &
                 units="nondim", default=.false.)
   
   ! NN_copy is a simple control to avoid network going out of bounds
   call get_param(param_file, mdl, "NEURAL_NETWORK", CS%NN_copy, &
                 "testing parameter file", &
                 units="nondim", default=.false.)
  
   ! lower limit of hbl, within 0 and hbl_lowlimit, shape function at 
   ! hbl_lowlimit will be used 
   call get_param(param_file, mdl, "NN_hbl_lowlimit", CS%NN_hbl_lowlimit, &
                 "lower limit of hbl under which shape function at this  & 
                 value will be used", units="metres", default=20.0)
   ! upper limit of hbl, from hbl_upplimit to bethymetry depth, shape 
   ! function at hbl_upplimit will be used
   call get_param(param_file, mdl, "NN_hbl_upplimit", CS%NN_hbl_upplimit, &
                 "upper limit of hbl over which shape function at this  &
                 value will be used", units="metres", default=1500.0)

   call get_param(param_file, mdl, "NEURAL_FILE", CS%NNfile, &
           "testing netcdf input", default="not_specified")

   call get_param(param_file, mdl, "NEURAL_VELOCITY_FILE", CS%NN_vel_file, &
           "netcdf file having neural net model of velocity scale", default="not_specified")
   
   call MOM_read_data(CS%NNfile,"forcing_mean",CS%NN_force_mean) ! 1d array
   call MOM_read_data(CS%NNfile,"forcing_std",CS%NN_force_std)   ! 1d array
   call MOM_read_data(CS%NNfile,"kappa_mean",CS%NN_kappa_mean)   ! 1d array
   call MOM_read_data(CS%NNfile,"kappa_std",CS%NN_kappa_std)     ! 1d array

   call MOM_read_data(CS%NNfile,"w1",CS%NN_w1,(/1,1,1,1/),(/4,32,1,1/))  ! 2d array
   call MOM_read_data(CS%NNfile,"b1",CS%NN_b1)  ! 1d array
   call MOM_read_data(CS%NNfile,"w2",CS%NN_w2,(/1,1,1,1/),(/32,32,1,1/))  ! 2d array
   call MOM_read_data(CS%NNfile,"b2",CS%NN_b2)  ! 1d array
   call MOM_read_data(CS%NNfile,"w3",CS%NN_w3,(/1,1,1,1/),(/32,16,1,1/))  ! 2d array 
   call MOM_read_data(CS%NNfile,"b3",CS%NN_b3)  ! 1d array
   
   !!! grabbing parameters of neural network velocity model
   call MOM_read_data(CS%NN_vel_file,"forcing_mean",CS%NN_vel_force_mean) ! 1d array
   call MOM_read_data(CS%NN_vel_file,"forcing_std",CS%NN_vel_force_std)   ! 1d array
   call MOM_read_data(CS%NN_vel_file,"nu_by_h_mean",CS%NN_vel_nubyh_mean)   ! 1d array
   call MOM_read_data(CS%NN_vel_file,"nu_by_h_std",CS%NN_vel_nubyh_std)     ! 1d array

   call MOM_read_data(CS%NN_vel_file,"w1",CS%NN_vel_w1,(/1,1,1,1/),(/3,32,1,1/))  ! 2d array
   call MOM_read_data(CS%NN_vel_file,"b1",CS%NN_vel_b1)  ! 1d array
   call MOM_read_data(CS%NN_vel_file,"w2",CS%NN_vel_w2,(/1,1,1,1/),(/32,32,1,1/))  ! 2d array
   call MOM_read_data(CS%NN_vel_file,"b2",CS%NN_vel_b2)  ! 1d array
   call MOM_read_data(CS%NN_vel_file,"w3",CS%NN_vel_w3,(/1,1,1,1/),(/32,2,1,1/)) ! 1d array  ,(/1,1,1,1/),(/64,1,1,1/))  ! 2d array
   call MOM_read_data(CS%NN_vel_file,"b3",CS%NN_vel_b3)  ! 1d array
   !!! neural netowkr velocity grabbed
